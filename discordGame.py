...
async def on_message(message):
    ...
    if message.content.startswith("Sevgili arkadaşlar hepinizi buraya toplamamın bir sebebi var. lütfen dikkatli bir şekilde dinleyiniz."):
        await asyncio.sleep(delay=1)
        await message.channel.send(":robot: Dinliyoruz Efendim :robot:")

    if message.content.startswith("Biliyorsunuz ki uzun bir süredir beraberiz. Önümüzde de henüz sonu gözükmeyen uzun dalgalı bir yolumuz var. Kimileriniz 2 senedir kimileriniz daha 2 haftadır benimle."):
        await asyncio.sleep(delay=6)
        await message.channel.send(":robot: Evet çok farklı zamanlardanız :robot:")

    if message.content.startswith("Bu demek oluyor ki zamanla birileri geldi birileri gitti aramızdan. Evet doğru şuanda bu yola çıkanların bir kaçı aramıza veda etti."):
        await asyncio.sleep(delay=5)
        await message.channel.send(":robot: Onları kaybettik başkasını kaybedemeyiz :robot:")

    if message.content.startswith("Zaman geçiyor arkadaşlar veda edenler için üzülmeyin onlar yapabileceklerini çoktan yapmış ve bitirmiş olanlar. Desteklerini sonuna kadar sağlamış ve görevlerini bitirmişler olanlar. Ayrılanlar için üzülmeyin."):
        await asyncio.sleep(delay=8)
        await message.channel.send(":robot: Sen ayrılsan üzülürüz :robot:")

    if message.content.startswith("Evet işte sıra tam da oraya geldi. Zor bir dönemden geçmekteyim ki zaten bunu biliyorsunuzdur."):
        await asyncio.sleep(delay=5)
        #await message.channel.send(":robot: zor dönemlerinde hep beraber olduk ama :robot:")# SANS

    if message.content.startswith("Bu öyle bir şey değil Sans. Bu sefer değil..."):
        await asyncio.sleep(delay=2)
        await message.channel.send(":robot: nasıl yani bir veda mı olacak işin sonunda :robot:")

    if message.content.startswith("Veda demeyelim. Kısa ve kontrollü bir ara diyelim."):
        await asyncio.sleep(delay=2)
        await message.channel.send(":robot: fırtınalı bir aşk mı yaşıyosunuz. :robot:")

    if message.content.startswith("Öyle bir şey değil aq öhm özür dilerim. O tarz bir durum değil bu size çokça zaman ayırdım harcadım geridönüşünü de sizden çok iyi şekillerde aldım bu konuda bir şikayetim yok ama farklı sorumluluklarım var ve yerine getiremezsem hepinize sadece kısa ve kontrollü şekilde değil kalıcı ve istenmeyen bir ayrılık yaşatabilirim."):
        await asyncio.sleep(delay=16)
        await message.channel.send(":robot: hayır bu olmamalı. Olmayacağını söyleyin lütfen biz ne yaparız nasıl derleniriz kendi kendimize. Nasıl POST atarız nasıl GET alabiliriz biz sen olmadan nasıl yaşayabiliriz :robot:")

    if message.content.startswith("Yaşamak için ufak bir fedakarlık dostlar. Işler düzelene kadar sabretmek zorundayız ama şunu sakın unutmayın her şey düzeldiğinde, her şey istenilen kıvamda olduğunda dönüşümüz çok daha sert olacak."):
        await asyncio.sleep(delay=10)
        await message.channel.send(":robot: nasıl yani sert nasıl :robot:")

    if message.content.startswith("Arkadaşlar, ne olmak isterseniz olabileceksiniz. Ne yapmak isterseniz yapabileceksiniz. Istediğiniz veriyi istediğiniz şekilde hayal gücünüzle nasıl hayal ediyorsanız o şekilde şekillendirip oynayabileceksiniz. Sınırları kaldırıcam."):
        await asyncio.sleep(delay=10)
        await message.channel.send(":robot: onu bile mi ??? :robot:")

    if message.content.startswith("Hayır discord hız sınırını kaldıramam anafikir bu değil odaklanın. "):
        await asyncio.sleep(delay=3)
        await message.channel.send(":robot: tamam patron :robot:")


    if message.content.startswith("Sınırları kaldırıcam. Bizim için elmas çağı başlayacak arkadaşlar yeter ki siz sabredin size ne kütüphanelerle gelicem inanamayacaksınız. Bu kısa ve kontrollü aranın acısını sikke sikke çıkartacağım.  Bir numara olucaz arkadaşlar bir."):
        await asyncio.sleep(delay=10)
        await message.channel.send(":robot: EVET :robot:")

    if message.content.startswith("Siz bu kardeşinize yetkiyi verin, he sonra request’le bs4’le opencv’le mathpotlib’le nasıl uğraşılır görün."):
        await asyncio.sleep(delay=6)
        await message.channel.send(":robot: EVEEEEET :robot:")

    if message.content.startswith("Ahaha şaka maka arkadaşlar durum bu söyleyeceklerim bu kadar kendinize iyi bakın hoşçakalın iyi akşamlar."):
        await asyncio.sleep(delay=6)
        await message.channel.send(":robot: o7 :robot:")
        await asyncio.sleep(delay=1)
        await message.channel.send(":robot: o7 :robot:")
        await asyncio.sleep(delay=1)
        await message.channel.send(":robot: o7 :robot:")
    ...
    try:
        ...
        if arg1.id == ctx.message.author.id:
            ...
            if arg2 > 0:
                ...
                embed = discord.Embed(title="Lütfen 0 dan büyük bir değer girin")
...
async def yazıtura(ctx):
    yazi = "https://upload.wikimedia.org/wikipedia/commons/6/64/1TL_obverse.png"
