...
@Bot.command(aliases=["s", "S", "Sorgulama", "sorgu", "Sorgu"])
@commands.has_role(852851191619911711)
async def sorgulama(ctx, arg1):
    file = open("databaseBot.txt", "r")

    contents = file.read()
    fiyat = ast.literal_eval(contents)

    file.close()
    await ctx.send(fiyat[arg1])
...
@Bot.command()
async def sayı(ctx, arg):
    a = random.randint(1, int(arg))

    b = 31 - a
    if a == 31:
        await ctx.send(f"{a}  sj")
    if a == 13:
        await ctx.send(f"{a} terse çevir sj")

    else:
        await ctx.send(f"{a}       + ({b})   sjsj")
